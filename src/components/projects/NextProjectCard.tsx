      ...
      <div className="mx-auto max-w-6xl">
        <div className="grid items-stretch gap-6 bg-(--project-next-card) p-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,.75fr)] lg:gap-0 lg:p-0">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 lg:p-6">
            <div>
              <p className="text-[16px] lg:text-[20px] font-normal uppercase leading-[100%] tracking-[-0.03em] text-(--color-foreground)">
                {label}
              </p>
              <h3 className="mt-4 text-[clamp(28px,4.6vw,56px)] font-semibold uppercase leading-[100%] tracking-[-0.03em] text-(--color-foreground)">
                {title}
              </h3>
            </div>
            <Link
          ...
          <div className="relative h-50 w-full lg:h-full">
            <Image
              src={image.src}
              alt={image.alt}
              fill
